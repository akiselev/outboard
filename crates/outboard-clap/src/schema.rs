...
pub struct CliCommandSchema {
    ...
    pub subcommands: Vec<CliCommandSchema>,
}
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
...
pub fn cli_schema<T: CommandFactory>() -> CliCommandSchema {
    cli_schema_from_command(T::command())
}
pub fn cli_schema_value<T: CommandFactory>() -> serde_json::Value {
    serde_json::to_value(cli_schema::<T>()).expect("schema serializes")
}
pub fn cli_schema_from_command(mut command: Command) -> CliCommandSchema {
    command.build();
    command_schema(&command)
}
fn command_schema(command: &Command) -> CliCommandSchema {
    let arguments = command
        .get_arguments()
        .map(|arg| CliArgSchema {
            ...
            value_names: arg
                .get_value_names()
                .map_or_else(Vec::new, |n| n.iter().map(ToString::to_string).collect()),
            possible_values: arg
                .get_possible_values()
                .into_iter()
    ...
    CliCommandSchema {
        ...
        version: command.get_version().map(ToOwned::to_owned),
        arguments,
