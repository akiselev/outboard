...
use outboard_clap::{ToArgv, validate_args, validate_parser};
use std::{ffi::OsString, path::PathBuf};
fn s(v: Vec<OsString>) -> Vec<String> {
    v.into_iter().map(|x| x.into_string().unwrap()).collect()
}
...
fn parser_roundtrip() {
    let cli = Cli {
        action: Action::Run(RunArgs {
            ...
            includes: vec!["one".into(), "two".into()],
            mode: Mode::Careful,
            jobs: 8,
            common: Common {
                verbose: true,
            ...
            trailing: vec!["--literal".into(), "value".into()],
        }),
    };
    assert_eq!(
        s(validate_parser(&cli).unwrap()),
        vec![
            "run",
            "input.pdf",
            "--output",
...
#[test]
fn delimiter_and_set_false() {
    assert_eq!(
        s(validate_args(&Delimited {
            values: vec!["a".into(), "b".into()],
            enabled: false
        })
        .unwrap()),
        vec!["--values", "a,b", "--enabled"]
    )
}
#[derive(Debug, Clone, PartialEq, Eq, Args, ToArgv)]
struct Constraint {
...
#[test]
fn clap_validates_constraints() {
    assert!(
        validate_args(&Constraint {
            alpha: Some("a".into()),
            beta: Some("b".into())
        })
        .is_err()
...
fn enum_shapes() {
    ...
    assert_eq!(
        s(Cli {
            action: Action::Named { label: "x".into() }
        }
        .to_argv()),
        vec!["named", "--label", "x"]
    )
